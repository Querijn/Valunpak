// unsigned long pack_rgba(): Helper method that packs RGBA channels into a single 4 byte pixel.
//
...
// void decompress_block_dxt1(): Decompresses one block of a DXT1 texture and stores the resulting pixels at the appropriate offset in 'image'.
//
    ...
    for (int j=0; j < 4; j++)
    {
        for (int i=0; i < 4; i++)
        {
            ...
            if (color0 > color1)
            {
                ...
                {
                    case 0:
                        finalColor = pack_rgba(r0, g0, b0, 255);
                        break;
                    case 1:
                        finalColor = pack_rgba(r1, g1, b1, 255);
                        break;
                    case 2:
                        finalColor = pack_rgba((2*r0+r1)/3, (2*g0+g1)/3, (2*b0+b1)/3, 255);
                        break;
                    case 3:
                        finalColor = pack_rgba((r0+2*r1)/3, (g0+2*g1)/3, (b0+2*b1)/3, 255);
                        break;
                }
            }
            else
            {
                switch (positionCode)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        finalColor = pack_rgba((r0+r1)/2, (g0+g1)/2, (b0+b1)/2, 255);
                        break;
                    case 3:
                        finalColor = pack_rgba(0, 0, 0, 255);
                        break;
                }
            }
...
// void block_decompress_image_dxt1(): Decompresses all the blocks of a DXT1 compressed texture and stores the resulting pixels in 'image'.
//
...
void block_decompress_image_dxt1(unsigned long width, unsigned long height, const unsigned char *blockStorage, unsigned long *image)
{
    ...
    {
        for (unsigned long i = 0; i < blockCountX; i++) decompress_block_dxt1(i*4, j*4, width, blockStorage + i * 8, image);
        blockStorage += blockCountX * 8;
    }
}

// void decompress_block_dxt5(): Decompresses one block of a DXT5 texture and stores the resulting pixels at the appropriate offset in 'image'.
//
// unsigned long x:                     x-coordinate of the first pixel in the block.
// unsigned long y:                     y-coordinate of the first pixel in the block.
// unsigned long width:                 width of the texture being decompressed.
...
void decompress_block_dxt5(unsigned long x, unsigned long y, unsigned long width, const unsigned char *blockStorage, unsigned long *image)
{
    ...
    for (int j=0; j < 4; j++)
    {
        for (int i=0; i < 4; i++)
        {
            ...
            {
                case 0:
                    finalColor = pack_rgba(r0, g0, b0, finalAlpha);
                    break;
                case 1:
                    finalColor = pack_rgba(r1, g1, b1, finalAlpha);
                    break;
                case 2:
                    finalColor = pack_rgba((2*r0+r1)/3, (2*g0+g1)/3, (2*b0+b1)/3, finalAlpha);
                    break;
                case 3:
                    finalColor = pack_rgba((r0+2*r1)/3, (g0+2*g1)/3, (b0+2*b1)/3, finalAlpha);
                    break;
            }
...
// void block_decompress_image_dxt5(): Decompresses all the blocks of a DXT5 compressed texture and stores the resulting pixels in 'image'.
//
// unsigned long width:                 Texture width.
// unsigned long height:                Texture height.
...
void block_decompress_image_dxt5(unsigned long width, unsigned long height, const unsigned char *blockStorage, unsigned long *image)
{
    ...
    for (unsigned long j = 0; j < blockCountY; j++)
    {
        for (unsigned long i = 0; i < blockCountX; i++) decompress_block_dxt5(i*4, j*4, width, blockStorage + i * 16, image);
