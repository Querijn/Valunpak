...
namespace valunpak
{
    ...
    bool test()
    {
        std::string name = "ShooterGame/Content/Equippables/Gun_UIData_Base";
        auto uasset = paks.get_file(name + ".uasset");

        if (uasset.first == nullptr)
            return false;

        ue4_uasset uasset_bin;
        if (uasset.first->get_file_data(*uasset.second, &uasset_bin) == false)
            return false;
        return true;
    }
}
